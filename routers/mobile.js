...
router.post('/mobile/image/:id', (req, res, next) => {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    var folderPath =
      path.join(__dirname, '../public/images') + '/' + req.params.id;
    ...
    let nextIdx = fs.readdirSync(folderPath).length + 1;
     ...
     * add new face
     */
    const faceList = JSON.parse(
      fs.readFileSync(path.join(__dirname, '../temp') + '/' + 'face-list.json')
    );

    faceList[req.params.id] = req.params.id;

    fs.writeFileSync(
      path.join(__dirname, '../temp') + '/' + 'face-list.json',
      JSON.stringify(faceList)
    );
